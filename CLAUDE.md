...
（それを増やすためのループなので）。
- **P2-a 部隊配置レイヤー（2026-09-15 決定）**: West Point アトラスから主要 10 作戦を軍/軍団レベル＋矢印で手打ち（`UnitSnapshot` / `Movement`、`docs/SCHEMA.md` 末尾）。師団日次（LoC 12th AG）と東部戦線の師団は今回やらない。元画像はリポに入れない
